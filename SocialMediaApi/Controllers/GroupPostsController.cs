...
namespace SocialMediaApi.Controllers
{
	...
	public class GroupPostsController : BasePostsController
	{
		...
		public async Task<ActionResult<Pagination<PostViewModel>>> GetPostsAsync([FromRoute] Guid groupId, int page = 1, int limit = 20, bool skipActivePosts = false)
		{
			...
			if (skipActivePosts)
			{
				return Ok(await _postService.GetPostsAsync(this.GetAuthUser(), groupId, page, limit));
			}
			var pageResult = await _activePostService.GetActivePostsAsync(this.GetAuthUser(), groupId, page, limit);
			if (pageResult.Results.Count() < (limit / 2))
			{
				pageResult = await _postService.GetPostsAsync(this.GetAuthUser(), groupId, page, limit);
			}
