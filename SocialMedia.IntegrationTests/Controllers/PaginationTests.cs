...
public class PaginationTests(IntegrationTestWebApplicationFactory factory) : BaseControllerTests(factory)
{
    ...
    public async Task GetPosts_ShouldReturnPagedResult()
    {
        ...
        for (int i = 0; i < 15; i++)
        {
            ...
            await _client.PostAsJsonAsync($"{Constants.ApiBase}/groups/{Constants.DefaultGroupId}/posts", command, TestContext.Current.CancellationToken);
        }
        ...
        // Act: Get Page 1
        var response1 = await _client.GetAsync($"{Constants.ApiBase}/groups/{Constants.DefaultGroupId}/posts?pageNumber=1&pageSize=10", TestContext.Current.CancellationToken);
        response1.EnsureSuccessStatusCode();
        var result1 = await response1.Content.ReadFromJsonAsync<PagedResult<PostDto>>(TestContext.Current.CancellationToken);

        // Act: Get Page 2
        var response2 = await _client.GetAsync($"{Constants.ApiBase}/groups/{Constants.DefaultGroupId}/posts?pageNumber=2&pageSize=10", TestContext.Current.CancellationToken);
