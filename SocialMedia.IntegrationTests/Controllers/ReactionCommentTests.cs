...
public class ReactionCommentTests(IntegrationTestWebApplicationFactory factory) : BaseControllerTests(factory)
{
    [Fact]
    public async Task ReactToComment_ShouldUpdateReadModel()
    {
        ...
        var postResponse = await _client.PostAsJsonAsync($"{Constants.ApiBase}/groups/{Constants.DefaultGroupId}/posts", postDto, TestContext.Current.CancellationToken);
        ...
        var commentResponse = await _client.PostAsJsonAsync($"{Constants.ApiBase}/comments", commentDto, TestContext.Current.CancellationToken);
        ...
        var reactionResponse = await _client.PostAsJsonAsync($"{Constants.ApiBase}/reactions/toggle", reactionCommand, TestContext.Current.CancellationToken);
        ...
        var getCommentsResponse = await _client.GetAsync($"{Constants.ApiBase}/comments/post/{postId}", TestContext.Current.CancellationToken);
    ...
    public async Task GetPost_ShouldReturnTop30Comments()
    {
        ...
        for (int i = 0; i < 40; i++)
        {
            ...
            await _client.PostAsJsonAsync($"{Constants.ApiBase}/comments", commentDto, TestContext.Current.CancellationToken);
        }
        ...
        var getCommentsResponse = await _client.GetAsync($"{Constants.ApiBase}/comments/post/{postId}?pageNumber=1&pageSize=30", TestContext.Current.CancellationToken);
        var commentsResult = await getCommentsResponse.Content.ReadFromJsonAsync<PagedResult<CommentReadDto>>(TestContext.Current.CancellationToken);
        ...
        var getMoreCommentsResponse = await _client.GetAsync($"{Constants.ApiBase}/comments/post/{postId}?pageNumber=2&pageSize=10", TestContext.Current.CancellationToken);
