...
public class ReactionCommentTests(IntegrationTestWebApplicationFactory factory) : BaseControllerTests(factory)
{
    ...
    public async Task ReactToComment_ShouldUpdateReadModel()
    {
        ...
        var commentResponse = await _client.PostAsJsonAsync("/api/v1/comments", commentDto, TestContext.Current.CancellationToken);
        ...
        var getCommentsResponse = await _client.GetAsync($"/api/v1/comments/post/{postId}", TestContext.Current.CancellationToken);
    ...
    public async Task GetPost_ShouldReturnTop30Comments()
    {
        ...
        var getCommentsResponse = await _client.GetAsync($"/api/v1/comments/post/{postId}?pageNumber=1&pageSize=30", TestContext.Current.CancellationToken);
        var commentsResult = await getCommentsResponse.Content.ReadFromJsonAsync<PagedResult<CommentReadDto>>(TestContext.Current.CancellationToken);
        ...
        var getMoreCommentsResponse = await _client.GetAsync($"/api/v1/comments/post/{postId}?pageNumber=2&pageSize=10", TestContext.Current.CancellationToken);
