...
namespace SocialMediaApi.Domain.Logic.Services
{
	public class ActivePostService : IActivePostService
	{
		...
		public async Task<PostViewModel?> GetActivePostAsync(AuthUser authUser, Guid ownerId, Guid id)
		{
		...
		public async Task<Pagination<PostViewModel>> GetActivePostsAsync(AuthUser authUser, Guid ownerId, int page = 1, int limit = 20)
		{
			var reactions = await UserDetailsReactionHelper.GetPostReactionsAsync(authUser, _userDetailsService);
