...
namespace SocialMediaApi.Domain.Logic.Services
{
	public class UserPostService : IUserPostService
	{
		...
		public UserPostService(SocialMediaApiDbContext dbContext, IUserDetailsService userDetailsService)
		{
		...
		public async Task<Pagination<PostViewModel>> GetUserPostsAsync(AuthUser authUser, Guid userId, int page)
		{
			var limit = 20;
			if (page <= 0)
			{
			...
			var reactions = await GetPostReactionsAsync(authUser);
		...
		private async Task<IList<MiniReaction>> GetPostReactionsAsync(AuthUser authUser)
		{
			var reactions = default(IList<MiniReaction>);
			if (authUser.IsAuthenticated)
			{
				reactions = await _userDetailsService.GetPostReactionsAsync(authUser);
			}
			else
			{
